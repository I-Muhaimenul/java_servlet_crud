...
public class UserDao {
    ...
    public void checkUser(User user) {
        try {
            PreparedStatement ps = connection.prepareStatement("select uname from user where uname = ?");
    ...
    public void addUser(User user) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("insert into user(uname, password, email, registeredon) values (?, ?, ?, ? )");
    ...
    public void updateUser(User user) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("update user set password=?, email=?, registeredon=?"
                    + "where uname=?");
            // Parameters start with 1
